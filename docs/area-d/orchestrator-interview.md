...
:::warning "INTERVIEW ← FIT"이 아니다
면접은 적합도(C)에 의존하지 않는다. 둘 다 JOB(B)에만 의존한다. 면접 질문 프롬프트에 적합도(C)·프로필(A)을 직접 주입하는 것은 아직 미연결이며(코드는 회사·직무·공고만 주입), 파이프라인 의존 그래프에서도 면접은 적합도와 형제(sibling)로 병렬이다.
:::
...
1. **질문 생성 입력 갭** — 문서는 면접 질문 입력으로 적합도(C)·프로필(A)을 약속하지만, 실제 `generateQuestions`는 회사·직무·모드·공고 원문(B)만 프롬프트에 주입한다. 같은 파이프라인에서 FIT(C)가 함께 돌아도 그 산출물이 INTERVIEW 프롬프트로 흘러들어가진 않는다.
2. **`prior` 활용 부분적** — 컨텍스트에 앞 단계 결과(`prior`)가 누적되어 전달되지만, INTERVIEW 핸들러는 현재 `prior`를 읽지 않는다. caseId만으로 면접 도메인이 알아서 공고 원문을 다시 읽는다(`accessService.sourceText`). 즉 의존은 "DB 커밋 순서" 보장용이지 "데이터 핸드오프"는 아직 caseId 경유.
3. **자체 LLM 생성 미가동** — 질문 생성은 사실상 Claude → OpenAI 폴백. 오케스트레이터가 호출해도 마찬가지(생성 task 화이트리스트가 빈 집합).
...
**3단계 (트레이드오프·정직):**
"두뇌가 LLM으로 모드·지원 건을 정하고 핸들러는 모드를 이중 방어합니다. 다만 솔직히 말하면, 같은 파이프라인에서 적합도(C)가 돌아도 그 결과가 면접 질문 프롬프트로 직접 들어가진 않습니다. 의존은 DB 커밋 순서 보장용이고, 면접은 caseId로 공고를 다시 읽습니다. 데이터 핸드오프까지 연결하는 게 다음 과제입니다."
