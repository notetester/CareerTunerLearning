...
`user_id`의 `UNIQUE`가 충돌 키다. 첫 저장은 INSERT, 이후는 같은 행을 UPDATE. **버전 행이 새로 쌓이지 않고 항상 같은 1행을 덮어쓴다** — 이 사실이 5절의 핵심이다.
...
private LocalDateTime updatedAt; // ★ createdAt 없음 = 버전 개념 아님, 현재 1행 평면화
```
...
여기가 면접에서 가장 점수를 따는 지점이다 — "계획 문서와 실제 코드가 다른 부분을 정직하게 구분"하는 능력.

### ★ user_profile_version — 계획에는 있으나 schema.sql에 없다

분담 계획표에는 `user_profile_version`(분석 재현성용 프로필 스냅샷) 테이블이 명시돼 있다. **그러나 `schema.sql`에 이 테이블은 존재하지 않는다.** 프로필은 4-2처럼 단일 행 upsert + `updated_at`만 있고, 시점별 버전 이력은 미구현이다.

:::warning 면접에서 "스냅샷 구현했나요?"를 받으면
"구현했다"고 답하면 사실 오류다. 정확한 답:
> "버전 스냅샷은 **설계 의도였고, 현재는 단일 행 upsert로 단순화**돼 있습니다. 관리자 화면의 '프로필 스냅샷'도 별도 버전 테이블이 아니라 현재 `user_profile` 1행을 평면화한 읽기 모델(`AdminUserProfileSnapshot`)입니다."
:::

### 왜 버전 스냅샷이 "원래" 핵심 설계였나 (개념은 이해해 둘 것)

면접관이 "그럼 버전이 왜 필요한 건데요?"로 파고들 수 있으니 의도는 또렷이 설명할 수 있어야 한다.

분석 재현성 문제: C가 "이 프로필로 적합도 78점"을 냈는데, 사용자가 그 후 프로필을 고치면 **그 78점이 어떤 프로필 기준이었는지 사라진다.** 버전 스냅샷이 있으면 "분석 시점의 프로필 N번 버전"을 고정 참조해 결과를 언제든 재현·해명할 수 있다.

현재 프로젝트는 이 문제를 다르게 푼다 — 버전 테이블 대신, **분석 결과 행에 입력 스냅샷을 JSON으로 동봉**한다. 예: C의 `fit_analysis.source_snapshot`, `career_analysis_run.input_snapshot`/`input_fingerprint`. 즉 "프로필 자체를 버전화"하는 대신 "분석이 자기가 쓴 입력을 박제"한다. 결과적으로 A에 버전 테이블이 없어도 재현성은 소비 측(C)에서 확보된다. ([캐싱·핑거프린트](/area-c/caching-fingerprint) 참고)
...
| 프로필 단일 행 upsert | 됨 | `ProfileMapper.xml` ON DUPLICATE KEY |
| JSON 8종 + 원문 2종 저장 | 됨 | `user_profile` 12개 필드 |
| 동의 append-only 이력 | 됨 | `user_consent` |
| 인증 감사 4종 분리 | 됨 | login/status/social/refresh 별도 테이블 |
| **`user_profile_version` 버전 테이블** | **미구현** | schema.sql에 없음 |
| 관리자 "프로필 스냅샷" | 됨(단, 버전 아님) | `AdminUserProfileSnapshot` = 현재 1행 평면화 |
...
3. **정직한 한계:** "분석 재현성을 위한 `user_profile_version` 버전 테이블은 계획엔 있었지만 실제로는 미구현이고, 프로필은 단일 행 upsert입니다. 대신 재현성은 소비 측에서 — C의 `fit_analysis.source_snapshot`처럼 분석 결과에 입력 스냅샷을 JSON으로 동봉하는 방식으로 확보합니다."
...
구분하지 않습니다. `ProfileMapper`의 `upsert`가 `INSERT ... ON DUPLICATE KEY UPDATE` 한 문장입니다. 충돌 키는 `user_id`의 UNIQUE 제약이고요. 첫 저장이면 INSERT, 이미 행이 있으면 같은 행을 UPDATE하면서 `updated_at = NOW()`를 갱신합니다. 그래서 회원 1명당 프로필 행은 항상 정확히 1개입니다.
:::

:::details Q4. 그럼 분석 재현성은 어떻게 보장하나요? 프로필이 바뀌면 과거 분석이 깨지지 않나요?
프로필 쪽에는 버전 이력이 없는 게 맞습니다. 재현성은 **소비 측에서** 확보합니다. C의 적합도 분석은 결과 행 `fit_analysis.source_snapshot`에, 장기 경향 분석은 `career_analysis_run.input_snapshot`/`input_fingerprint`에 "그 분석이 실제로 쓴 입력"을 JSON으로 박제합니다. 프로필 원본이 나중에 바뀌어도 분석 결과는 자기가 쓴 입력 스냅샷을 그대로 들고 있어 재현·해명이 됩니다. 핑거프린트는 같은 입력이면 캐시를 재사용하는 키로도 쓰입니다.
:::

:::details Q5. 관리자 화면의 "프로필 스냅샷"은 버전 테이블 아닌가요?
아닙니다. `AdminUserProfileSnapshot`은 별도 `user_profile_version` 테이블 조회가 아니라 **현재 `user_profile` 1행을 평면화한 읽기 모델**입니다. 모든 JSON 컬럼이 `String`으로 내려오고, `updatedAt`만 있고 버전 번호나 다중 행 개념이 없습니다. "스냅샷"이라는 이름 때문에 버전 이력으로 오해하기 쉬운데, 실제로는 현재 상태의 단일 뷰입니다.
:::
...
3. 프로필 저장이 **왜 upsert 한 방**인지, 그래서 버전이 안 쌓인다는 사실
4. 버전 스냅샷이 **미구현인데도** 재현성이 깨지지 않는 이유 (소비 측 입력 박제)
...
<QuizBox question="user_profile_version(분석 재현성용 프로필 버전 스냅샷) 테이블에 대한 가장 정확한 설명은?" :choices="['schema.sql에 정의돼 있고 매 프로필 저장마다 새 버전 행을 쌓는다', '계획에는 있으나 schema.sql에 없으며, 프로필은 단일 행 upsert로 단순화돼 있다', '관리자 AdminUserProfileSnapshot이 곧 이 버전 테이블이다', 'C 영역이 소유하며 A는 참조만 한다']" :answer="1" explanation="user_profile_version는 분담 계획에는 있으나 실제 schema.sql에는 없다. 프로필은 ON DUPLICATE KEY UPDATE 단일 행 upsert이고, 재현성은 C의 fit_analysis.source_snapshot 등 소비 측 입력 박제로 확보한다. AdminUserProfileSnapshot은 현재 1행을 평면화한 읽기 모델이지 버전 테이블이 아니다." />
