...
스킬 추출은 진단 결과를 **캐시 테이블에 저장하지 않는다.** AI가 돌면 `ai_usage_log`에 호출 사실(featureType·status·model·token·`creditUsed=0`)만 append할 뿐, `extractedSkills` 배열 자체는 응답으로만 내려가고 사라진다. 이유: 추출은 "지금 프로필 기준의 제안"이라 프로필이 바뀌면 매번 다시 뽑는 게 맞고, 별도 저장은 사용자 확정값(`skills`)과 어긋난 **두 번째 진실원**을 만든다.
...
| `RuleBasedProfileAiService` | 결정론적 추출 `extractSkillNames(profile)` (운영 기본값) |
...
- **키 미발급(운영 기본값):** 즉시 `RuleBasedProfileAiService`로 위임. `status="SUCCESS"`, `model="profile-rule-v2"`.
...
POST /api/profile/ai/skills
  ...
  → ProfileAiResponse { extractedSkills[], ... }   (DB에 결과 저장 X)
```
...
| 규칙엔진 추출(`extractSkillNames`) | 구현됨, 운영 기본값으로 실제 동작 |
| 사전 기반 정규 표기 통일 + `LinkedHashSet` 중복 제거 | 구현됨 |
| LLM 추출(Responses API, strict schema) | 구현됨이나 **키 미발급이라 비활성**, 폴백으로 보호 |
| `extractedSkills` → 프로필 자동 반영 | **미구현(의도적 부재)**. 표시 전용 |
| 추출 결과 저장(캐시 테이블) | **없음**. `ai_usage_log`에 호출 메타만 기록 |
| 의미 기반 동의어 통합·오타 교정 | **없음**. 사전·결정론적 정리만 |
| 자체 파인튜닝 추출 모델 `careertuner-a-profile-3b`(Qwen2.5-3B LoRA) | **설계만**, 키 미발급. 점수 합산은 어차피 서버 소유 |
...
저장되지 않습니다. `extractedSkills`는 응답 DTO에만 담겨 내려가고, AI가 돌았다는 사실만 `ai_usage_log`에 `featureType=PROFILE_SKILL_EXTRACT`, `creditUsed=0`으로 append됩니다. 사용자가 확정한 스킬만 `user_profile.skills`(JSON 컬럼)에 PUT으로 저장됩니다. 결과 캐시 테이블은 없습니다.
:::
...
3. 추출 결과가 프로필에 저장되지 않는다는 사실과 그 이유를, A의 데이터 경계와 엮어서.
...
<QuizBox question="스킬 추출 전용으로 별도 존재하는 것은 무엇인가?" :choices="['전용 엔드포인트, 전용 테이블, 전용 모델이 모두 따로 있다', '전용 featureType만 있고 테이블·모델은 공용을 재사용한다', '아무것도 없고 요약 엔드포인트에 통합돼 있다', '전용 결과 캐시 테이블만 있다']" :answer="1" explanation="POST /profile/ai/skills와 featureType=PROFILE_SKILL_EXTRACT는 스킬 추출용으로 분기하지만, 저장은 공용 user_profile.skills 컬럼, 추적은 공용 ai_usage_log, AI 파이프라인도 요약·완성도진단과 공유한다. 전용 테이블·전용 운영 모델은 없다(파인튜닝 모델은 설계만)." />
