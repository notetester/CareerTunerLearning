...
:::tip 현재 상세와 버전 이력은 별도다
`AdminUserProfileSnapshot`은 현재 `user_profile` 1행을 평면화한 읽기 모델이다. 별도로 `/api/admin/profiles/{userId}/versions`가 `user_profile_version` 이력을 반환하고 관리자 프로필 화면이 둘을 함께 보여준다. 자세히는 [프로필 불변 버전](/area-a/profile-versioning) 참고.
:::
...
| 회원 상세의 현재 프로필 표시 | 구현됨(`AdminUserProfileSnapshot`) |
| 관리자 프로필 버전 목록 | 구현됨(`/admin/profiles/{userId}/versions`) |
| 관리자의 프로필 **수정** | 미구현(설계상 읽기 전용 — 본인만 수정) |
| 관리자의 동의 **수정/철회 대행** | 미구현(append-only라 개념상 미제공) |
| 관리자의 프롬프트 **편집** | 미구현(코드 상수라 배포로만 변경) |
| 버전 목록 조회 | 구현됨, 현재 상세와 source/version/createdAt 대조 |
| 과거 버전을 현재 프로필로 복원하는 관리자 쓰기 | 제공하지 않음 |
...
<QuizBox question="관리자 프로필의 현재 상세와 버전 이력 관계로 옳은 것은?" :choices="['AdminUserProfileSnapshot 하나가 모든 버전 목록이다', '현재 상세는 user_profile 읽기 모델이고 버전 목록은 별도 versions API로 조회한다', '관리자는 프로필 이력을 볼 수 없다', '버전은 브라우저에서 계산한다']" :answer="1" explanation="현재 1행 상세와 불변 user_profile_version 목록은 별도 응답이며 AdminProfilesPage가 함께 조회해 보여준다." />
