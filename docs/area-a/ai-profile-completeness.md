...
또 하나의 핵심 트레이드오프는 **결과를 저장하지 않는다**는 것이다. 진단 점수·기준별 채점 결과는 응답으로만 내려가고 DB에 캐시 테이블이 없다. 남는 것은 `ai_usage_log` 한 줄(호출 추적)뿐이다. 장점은 단순함과 "항상 최신 프로필 기준"이라는 점, 단점은 점수 추이 비교(어제 60점 → 오늘 75점)를 서버가 보장하지 못한다는 점이다.
...
| `RuleBasedProfileAiService` | 결정론적 채점. **운영 기본값**(키 미발급) |
...
- **운영 기본값은 규칙엔진:** OpenAI 키가 미발급이라 `openAiClient.configured()`가 false → 항상 `RuleBasedProfileAiService`가 실제로 동작한다(`model="profile-rule-v2"`, `status="SUCCESS"`). LLM 경로는 키 주입 시에만 활성.
- **결과 캐시 없음:** 진단 점수는 DB에 저장되지 않는다. "지난 진단과 비교" 기능은 없다.
- **자체 파인튜닝 모델은 설계만:** `careertuner-a-profile-3b`(Qwen2.5-3B LoRA, 추출형)는 1차 경로로 설계됐으나 미발급 상태라 미동작. 폴백 구조로 보호.
- **점수 이력/추이 테이블 없음:** `user_profile_version`(분석 재현성 스냅샷)은 schema.sql에 존재하지 않는다. 프로필은 단일행 upsert + `updated_at`만 있다.
:::
...
**3단계(트레이드오프·정직):** "운영 기본값은 키 미발급이라 규칙엔진이 실제로 동작하고, LLM은 폴백 구조 안에서 키가 들어오면 활성화됩니다. 동의(AI_DATA)가 없으면 진단 자체를 막고, 결과는 캐시 없이 매번 최신 프로필 기준으로 다시 계산합니다. 그래서 점수 추이 비교는 아직 못 하는 게 한계입니다."
...
동의는 실행 전제 조건이다. `requireAiConsent`가 `AI_DATA` 현재 동의(같은 type 최신 1행이 agreed=true이고 revoked_at이 null)를 확인하고, 없으면 FORBIDDEN으로 진단을 막는다. 화면은 실패 시 "AI 데이터 동의 상태를 확인해 주세요"를 안내한다. 최근 수정은 별도 타임스탬프 비교가 아니라, 진단이 매번 현재 `user_profile` 1행을 그대로 읽어 평가하므로 자동으로 최신 입력이 반영되는 구조다(결과 캐시가 없어 옛 점수가 끼어들지 않는다).
:::
...
5. 동의 게이트와 "결과 캐시 없음"이 진단 신선도에 주는 영향.
