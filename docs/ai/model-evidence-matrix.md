...
C가 적응에 쓴 베이스는 Qwen2.5-3B이고, 이 베이스의 공식 메타데이터 라이선스는 **연구용(비상업)** 조건이다. LoRA 어댑터는 직접 학습했지만 병합 모델은 베이스 가중치를 포함하므로 그 제약을 물려받는다. 따라서 "자체 모델을 학습했다"가 곧 "상용 기본값으로 쓸 수 있다"를 뜻하지 않는다. 상용 경로는 (1) 상업 친화 라이선스 베이스 재학습, (2) 외부 provider 기본 사용, (3) 라이선스 정리 후 승격 중 하나로 풀어야 하며, 이는 현재 기본 provider가 외부인 이유 중 하나다. 면접에서는 "학습 완료"와 "상용 승격 가능"을 반드시 구분해서 말한다.
