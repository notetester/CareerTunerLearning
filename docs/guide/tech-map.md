...
:::warning 정직한 구분 (A)
AI 엔드포인트는 `summary`/`skills`/`completeness` **3개**다. 자소서·경력 키워드(#3·#4)는 요약 응답 필드에 통합됐다. 프로필 AI는 설정된 자체 LoRA 경로와 Claude→OpenAI→규칙 안전망을 구분하며, 사용자가 모델 tier를 선택할 수 있다. → [키워드 추출](/area-a/ai-keyword-extraction)
:::
...
:::tip 현재 연결 (E)
첨삭 프런트는 생성·목록·상세·소프트 삭제 API와 모델 선택을 사용하고, 유효 결과 뒤 `AiChargeService`가 실제 사용권·크레딧을 정산한다. #28도 `BillingServiceImpl.recommendPlan`과 Billing 추천 카드가 연결돼 있다. → [요금제 추천](/area-e/ai-plan-recommend)
:::
...
| --- | --- | --- | --- |
| SPA | React 19 + Vite 8 + TypeScript | `frontend/src/features/<기능>/{pages,components,api,hooks,types}` | [React](/frontend/react)·[Vite](/frontend/vite) |
