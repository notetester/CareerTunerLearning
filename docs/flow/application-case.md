...
표는 "이 단계가 어느 영역·어느 테이블에 무엇을 남기고, 다음 단계에 무엇을 넘기는가"를 정리한 것이다. 전체 약 68개 테이블이 있지만 지원 건 줄기에 직접 매달리는 것만 추렸다.
...
| 인테이크 챗봇(F) → AutoPrep 연결 | 백엔드 구현·**프론트 미연결** | `ai/intake` 패키지, 슬롯은 JVM 인메모리 |
