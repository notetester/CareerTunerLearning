...
> React는 UI를 상태(state)의 함수로 보고, 상태가 바뀌면 화면을 자동으로 다시 그려주는 선언형 컴포넌트 라이브러리입니다. CareerTuner 프론트엔드 전체가 React 18 함수형 컴포넌트 + Hooks로 작성돼 있습니다.
...
CareerTuner 프론트엔드는 `frontend/src/features/<기능>/{api,components,hooks,pages,types}` 구조로 기능별 수직 분리돼 있고, 전부 React 18.3.1 함수형 컴포넌트다.
