...
### 4-4. 리뷰-우선 근거 게이트와 3계층 역할분담

앞의 두 가드는 "위반이면 재호출하거나 강등"하는 즉시 처리다. 여기서 한 걸음 더 나아가면, 위반을 바로 버리거나 통과시키는 대신 **상태로 남겨 사람이 검수**하게 만들 수 있다. 이것이 리뷰-우선(review-first) 근거 게이트의 개념이다.

- 게이트는 출력을 세 상태 중 하나로 판정한다: **통과(PASSED) · 검수 필요(REVIEW_REQUIRED) · 거부(REJECTED)**.
- `REVIEW_REQUIRED`는 관리자 큐로 들어가 사람이 확인한다. "애매하면 조용히 통과"가 아니라 **"애매하면 표시하고 멈춘다"**가 기본값이다.
- 상태 판정은 결정적이라 같은 입력이면 같은 상태가 나오고, 감사·재현이 된다.

왜 이렇게까지 하나. 실제로 근거 이탈은 파인튜닝을 해도 사라지지 않았다 — 오히려 도메인 LoRA가 베이스보다 근거 이탈을 **더** 낸 측정 결과도 있었다(60케이스에서 베이스 6건 대 LoRA 11건). 즉 **모델을 더 학습시켰다고 사실성이 저절로 보장되지 않는다.** 그래서 역할을 세 층으로 못 박는다.

| 층 | 담당 | 무엇을 보장하나 |
| --- | --- | --- |
| **스키마(구조)** | JSON 스키마·문법 강제 | 형식·필드 존재를 보장 (구조 보장) |
| **결정적 근거 게이트** | grounding 검사 + 리뷰-우선 상태머신 | 입력에 없는 사실을 차단 (사실 차단) |
| **LoRA(설명)** | 도메인 파인튜닝 | 전략 설명의 구체성·어조 (전략 구체성) |

핵심은 **사실 차단을 모델이 아니라 결정적 게이트가 맡는다**는 것이다. LoRA에는 "더 그럴듯하게 설명하기"만 맡기고, "그 설명이 사실인지"는 코드가 판정한다.

:::tip 방어선이 겹쳐 있다
(1) 점수·판단을 애초에 모델에 맡기지 않고(규칙엔진 소유), (2) 모델이 금지 필드를 내도 안 읽고(화이트리스트), (3) 사실 이탈은 근거 게이트가 잡고, (4) 애매한 건 사람 검수 큐로 보낸다. 어느 한 겹을 뚫어도 다음 겹이 남는다. 프롬프트 주입 공격에도 점수·판정이 흔들리지 않는 이유는 [뉴로-심볼릭 설계](/area-c/neuro-symbolic)에서 다룬다.
:::
